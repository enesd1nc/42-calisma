...
static int	handle_flood_and_checks(t_flood *flood, char **map, int *visited,
		int *visited_exit)
{
	t_check_info	check = {.map = map, .visited = visited,
			.visited_exit = visited_exit, .width = flood->width, .height;
	int				result;

	perform_flood_fills(flood, visited_exit);
	check = {.map = map, .visited = visited, .visited_exit = visited_exit,
		.width = flood->width, .height = flood->height};
...
int	check_collectibles(char **map, int width, int height)
{
	...
	int		exit_found;
	t_pos	player;
	t_flood	flood = {.map = map, .x = player.x, .y = player.y,
	...
	flood = {.map = map, .x = player.x, .y = player.y, .visited = visited,
		.width = width, .height = height, .check_collectibles = 1};
