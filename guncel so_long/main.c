...
int close_window(t_data *data)
{
    ...
    exit(0);// return den farkı komple programı kapatır.
    return (0);
}
    ...
    if (data->map[new_y][new_x] != '1')
    {
        ...
        if (data->map[new_y][new_x] == 'E')
        {
            if (data->collectibles == 0)
            {
                ...
                move++;
                printf("%d\n%s\n", move, "Tüm collectible'lar toplandı, oyun bitti!");
                close_window(data);
            }
            else
            {
...
int main(int argc, char *argv[])
{
    ...
    {
        close_window(&data); // Tüm belleği temizleyen fonksiyonu kullan
        return (1);
    }

    // Collectible sayısını hesapla
    data.collectibles = 0;
    int y = 0;
    while (data.map[y])
    {
    ...
    // XPM dosyalarını yükle ve kontrol et xpm dosyasını imageye donusturur.
